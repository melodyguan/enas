...
class PTBRhnModel(object):
  ...
  def _rhn(self, x, prev_s, w_x, w_rhn, s_mask=None):
    s = prev_s
    for layer_id, w in enumerate(w_rhn):
      if s_mask is not None:
        ht = tf.matmul(s * s_mask, w)
      else:
        ht = tf.matmul(s, w)
      if layer_id == 0: ht += tf.matmul(x, w_x)
  ...
  def _model(self, x, is_training, is_test):
    ...
    def body(step, prev_h, all_h):
      next_h = []
      for layer_id, (p_h, w_x, w_rhn) in enumerate(zip(prev_h, self.w_x, self.w_rhn)):
        if layer_id == 0:
        ...
        curr_h = self._rhn(inputs, p_h, w_x, w_rhn,
  ...
  def _build_params(self):
    ...
    with tf.variable_scope(self.name, initializer=initializer):
      with tf.variable_scope("variational_rhn"):
        self.w_x, self.w_rhn = [], []
        for layer_id in xrange(self.lstm_num_layers):
          with tf.variable_scope("layer_{}".format(layer_id)):
            w_x = tf.get_variable("w_x", [self.lstm_hidden_size, 2 * self.lstm_hidden_size])
            self.w_x.append(w_x)
            w_rhn = []
            for rhn_layer_id in xrange(self.rhn_depth):
              with tf.variable_scope("rhn_{}".format(rhn_layer_id)):
                w = tf.get_variable("w", [self.lstm_hidden_size, 2 * self.lstm_hidden_size])
                w_rhn.append(w)
            self.w_rhn.append(w_rhn)
